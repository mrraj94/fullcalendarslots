...
import fetchcalendarevent from '@salesforce/apex/FullCalendarController.fetchcalendarevent';
 ...
 */
export default class fullcalendarPOC extends LightningElement {
    ...
    eventObj(value){
        ...
        if(data){
            ...
            let events = data.map(event => {
                console.log('Select_Slot__c', event.Select_Slot__c);
                let clr = '';
                if(event.Select_Slot__c==true){
                    clr = '#ed391a';
                }else{
                    clr = '#378006';
                ...
                return { id : event.Id, 
                    ...
                    selectSlot : event.Select_Slot__c,
   ...
   renderedCallback() {
    ...
    initialiseFullCalendarJs() {
        ...
        function openActivityForm1(startDate, endDate, isSelected, titletext){
            self.startDate = startDate;
            self.endDate = endDate;
            self.openModal = true;
            self.selectSlot = isSelected;
        ...
        $(ele).fullCalendar({
            ...
            eventClick: function (event) {
                let stDate = event.start.format();
                let edDate = event.end.format();
                let isSelected = event.selectSlot;
                let titletext = event.title;
                
                self.selectedEvent = event;
                // openActivityForm(event.start, event.end);
                if(isSelected==false && event.start>=new Date()){
                    openActivityForm1(stDate, edDate, isSelected, titletext);
                }
                // openActivityForm1();
                // self.openModal = true;
    ...
    handleCheckbox(event){
        this.selectSlot = event.target.checked;
    ...
    handleSave(event) {
        ...
        this.template.querySelectorAll('lightning-input').forEach(ele => {
           ...
           if(ele.name === 'start'){
            ...
            //     // this.selectSlot = ele.value;
            // }
        });
       console.log('startdate-',this.startDate);
        //format as per fullcalendar event object to create and render
        let newevent = {title : this.title, start : this.startDate, end: this.endDate, selectSlot: this.selectSlot};
        console.log('-----New Event', newevent);
        console.log('Selected Event : ', this.selectedEvent);
        console.log('selected slot? - ', this.selectSlot);
        ...
        .catch( error => {
            ...
            //Update the slot
            console.log('=================Update the fucking slot', this.selectedEvent.id);
            //To close spinner and modal
            // this.openSpinner = false;
            newevent.id = this.selectedEvent.id;
            console.log('new event : ', newevent);
            let eventid = this.selectedEvent.id;
            this.selectedEvent.selectSlot = this.selectSlot;
            updateEvent({'eventid' : eventid, 'isSelected':this.selectSlot})
            .then( result => {
                console.log(result);
                const ele = this.template.querySelector("div.fullcalendarjs");
                console.log('inside update : ', eventid);
                this.selectedEvent.color = '#ed391a';
        ...
        fetchcalendarevent({StartDateTime:this.startDate}).then(
            data =>{
                console.log('total slots -',data);
            }
        ).catch(error =>{
